		...
		if (this.stab) {
			...
			if (this.weapon == 8) {
				for(a=0;a<bullets.length;a++) {
					for (b=0;b<this.len;b+=20) {
						x = player.x+w/2 + b * Math.round(cos(radians(this.dir)));
						y =  player.y+w/2 + b *Math.round(sin(radians(this.dir)));
						if (dist(bullets[a].x,bullets[a].y,x,y) <= w) {
