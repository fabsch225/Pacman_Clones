...
/*	if (keyIsDown(49) && player.inv[0] != "free" && !String(player.inv[0]).includes("k")) {
	...
	if (keyIsDown(49) && !String(player.inv[1]).includes("k")) {
		...
		if (player.inv[1] != "free") {
			reload = weapons[player.inv[1]][0]/10;
		}
	}
	...
	if (keyIsDown(50) && !String(player.inv[2]).includes("k")) {
		...
		if (player.inv[2] != "free") {
			reload = weapons[player.inv[2]][0]/10;
		}
	}
	...
	if (keyIsDown(51) && !String(player.inv[3]).includes("k")) {
		...
		if (player.inv[3] != "free") {
			reload = weapons[player.inv[3]][0]/10;
		}
	}
	...
	if (keyIsDown(81) && player.inv.includes(player.weapon) ) {
		...
		var ind;
		if (d == 1) {
			ind = index(Math.floor(player.x/w),Math.floor(player.y/w)) + 1
		}
		else if (d == 2) {
			ind = index(Math.floor(player.x/w),Math.floor(player.y/w)) - 1
		}
		else if (d == 3) {
			ind = index(Math.floor(player.x/w),Math.floor(player.y/w)) - cols
		}
		else if (d == 4) {
			ind = index(Math.floor(player.x/w),Math.floor(player.y/w)) + cols
		}
		console.log(player.weapon, mouseButton)
...
function showAll() {
			...
			if (player.inv[a] != "free") {
				if (player.weapon == player.inv[a]) {
					if (!String(player.weapon).includes("k")) {
						...
						stroke("blue")
						rect(wi, rows*w-9, textWidth(weapons[player.inv[a]-1][1]), w)
						stroke("yellow")
						rect(wi, rows*w-9, textWidth(weapons[player.inv[a]-1][1])-p, w)
						
						text(weapons[player.inv[a]-1][1], wi, rows*w+w-9)
						...
						stroke("blue")
						rect(wi, rows*w-9, textWidth(swords[wep][1]), w)
						stroke("yellow")
						rect(wi, rows*w-9, textWidth(swords[wep][1])-p, w)
					
						text(swords[wep][1], wi, rows*w+w-9)
					...
					if (!String(player.inv[a]).includes("k")) {
						...
						rect(wi, rows*w-9, textWidth(swords[parseInt(player.inv[a])][1]), w)
						text(swords[parseInt(player.inv[a])][1], wi, rows*w+w-9)
	...
	let pox = 0
	let poy = 0
	for (a = 0; a < room_cols; a++) {
		for (b = 0; b < room_rows; b++) {
			...
			rect(pox,poy,w,w)
		//	text(ghosts_ch[pos].length, pox, poy+(w))
	...
	v1 = createVector(player.x+w/2,0);

    v0 = createVector(player.x+w/2, player.y+w/2);
    v2 = createVector(mouseX-(player.x+w/2), mouseY-(player.y+w/2));
